...
exports.obtenerContratos = async (event) => {
   ...
   try {
      ...
      let page = await setupPage(browser, fechaInput);
      ...
      // Recarga la página para que la app lea el localStorage
      ...
      if (dataGlobal.length === 0) {
         const initialTableData = await todasLasPaginas(page);
         console.log("Total --> ", initialTableData.length);
         total = await textoTotal(page);
         if (initialTableData.length > 0) {
            dataGlobal.push(...initialTableData);
         } else {
            await browser.close();
            return response;
         }
      } else {
         console.log(`dataGlobal ya tiene ${dataGlobal.length} items`);
      }
      if (total > 0) {
         await page.goto('https://comprasmx.buengobierno.gob.mx/sitiopublico/#/', {
            waitUntil: 'networkidle2',
         ...
         for (let i = posicionActual; i < dataGlobal.length; i++) {
            ...
            if (paginaDeseada > 1) {
               const selectorPagina = `p-paginator .p-paginator-pages button[aria-label="${paginaDeseada}"]`;
               try {
                  ...
                  if (!estaSeleccionado) {
                     await page.waitForSelector(selectorPagina, { timeout: 10000 });

                     await Promise.all([
                        page.waitForNetworkIdle({ idleTime: 500, timeout: 10000 }).catch(() => console.warn("Network idle timeout al cambiar de página")),
                        page.click(selectorPagina)
                     ]);
            ...
            try {
               ...
               console.log(LogError("extractDetails", extractError, { URL: dataGlobal[i].URL }));
               ...
               dataGlobal[i].detalles = { error: dataError.message };
               dataGlobal[i].URL = dataError.URL;
               posicionActual++;
            ...
            // Esperamos que la PÁGINA 1 se cargue
            await waitForRows(10, 500, page);

            console.log(`Progreso: ${i + 1} / ${dataGlobal.length}`);
            posicionActual++;
            if (contador === 100 && i < (dataGlobal.length - 1)) {
               await browser.close();
               const fecha = obtenerNombreArchivo(fechaInput);
               const data = {
                  fecha,
...
async function setupPage(browser, fechaInput) {
...
function obtenerFechaAyer(fechaInput) {
   if (fechaInput && fechaInput !== null && (fechaInput.hasOwnProperty("fechaInicial") || fechaInput.hasOwnProperty("fechaFinal"))) {
      const fechaInicial = fechaInput.hasOwnProperty("fechaInicial") ? fechaInput.fechaInicial : fechaInput.fechaFinal;
      const fechaFinal = fechaInput.hasOwnProperty("fechaFinal") ? fechaInput.fechaFinal : fechaInput.fechaInicial;
   ...
   return { fechaInicial: fechaAyer, fechaFinal: fechaAyer };
}

function obtenerNombreArchivo(fechaInput) {
   const fechas = obtenerFechaAyer(fechaInput);
   if (fechas.fechaInicial !== fechas.fechaFinal) {
      return `${fechas.fechaInicial} ~ ${fechas.fechaFinal}`;
   }
   return `${fechas.fechaInicial}`
}

function textoTotal(page) {
   return new Promise(async (resolve, reject) => {
      try {
         await page.waitForSelector('app-sitiopublico-tabla', { timeout: 15000 });
         await page.waitForFunction(
            () => {
               const labels = document.querySelectorAll('app-sitiopublico-tabla label');
               return Array.from(labels).some(label => label.textContent.includes('Total:'));
            },
            { timeout: 10000 }
         );
         const total = await page.evaluate(() => {
            const component = document.querySelector('app-sitiopublico-tabla');
            ...
            if (targetLabel) {
               const boldElement = targetLabel.querySelector('b');
               return boldElement ? boldElement.textContent.trim() : null;
            }
         ...
         if (total === null) {
            console.log('No se encontró el elemento del total');
            resolve(0)
         }
...
async function todasLasPaginas(page) {
   let allData = [];
   ...
   while (true) {
      ...
      const isNextDisabled = await page.evaluate(() => {
         const nextButton = document.querySelector('p-paginator .p-paginator-next');
         if (!nextButton) return true;
         return nextButton.classList.contains('p-disabled') || nextButton.hasAttribute('disabled');
      });

      if (isNextDisabled) {
         console.log("El botón 'Siguiente' está deshabilitado. Terminando.");
         break;
      }
      console.log("Haciendo clic en 'Siguiente'...");

      await page.click('p-paginator .p-paginator-next');
      try {
         await page.waitForNetworkIdle({ idleTime: 500, timeout: 5000 });
      } catch (e) {
         console.warn("waitForNetworkIdle tardó mucho, continuando de todos modos.");
      }
   }

   console.log(`Scraping completo. Total de registros: ${allData.length}`);
   return allData;
}
         ...
         if (requirementsNode) {
            ...
            if (economicContainer) {
               ...
               if (partidaContainers.length > 0) {
                  ...
                  Array.from(partidaContainers).slice(0, 3).forEach(container => {
                  ...
                  Array.from(tables).slice(0, 3).forEach((table, index) => {
                     const tableData = extractTableData(table);
                     if (tableData.length > 0) {
                        if (!scrapedData.hasOwnProperty("Requerimientos")) {
                           scrapedData["Requerimientos"] = {};
